...
FIG_DIR = PROJECT_ROOT / "figures" / "20260226_PPO_Return_FineTune"
FIG_DIR.mkdir(parents=True, exist_ok=True)
LOG_DIR = PROJECT_ROOT / "runs" / "20260226_PPO_Return_FineTune"
...
def ppo_update(
    ...
    for _ in range(ppo_epochs):
        ...
        for start in range(0, n, mini_batch_size):
            ...
            # Freeze Actor weights if specified
            if not freeze_actor:
...
def train(args) -> None:
    ...
    target_critic = PPOCritic(state_size).to(device)
    ...
    # === [MODIFIED] Actor-Only Transfer + Alpha Reset ===
    if getattr(args, "init_from", None):
        ...
        # Load Actor ONLY (Critic stays randomly initialized)
        smart_load_weights(actor, ckpt_path, device, key='actor')
        # Critic: keep random init (no transfer — avoids Q-value distortion)
    ...
    for episode in range(start_episode, start_episode + int(args.episodes)):
        ...
        def get_lr(base_lr: float, episode: int) -> float:
            ...
            return max(base_lr * (1.0 - progress) + getattr(args, "min_lr", 1e-5) * progress, getattr(args, "min_lr", 1e-5))
        ...
        while not done and steps < args.max_steps:
            with torch.no_grad():
                ...
                action_env = np.asarray(action_np, dtype=np.float32).flatten()
            ...
            rollout_buffer.push(state.squeeze(0), action_env, log_prob, float(reward), bool(done), value)
            state = next_state
            score += reward
            steps += 1
        ...
        if last_actor_loss is not None:
            ...
            writer.add_scalar("episode/entropy", last_entropy if last_entropy is not None else 0.0, episode)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--freeze_actor_episodes", type=int, default=0, help="Episodes to freeze Actor weights for learning Critic Value only first.")
    parser.add_argument(
        "--max_throttle",
        type=float,
        default=1.0,
    ...
    parser.add_argument(
        ...
        help="Path to pretrained model checkpoint to load weights from",
    )
