...
SCRIPT_TAG = "20260310_PPO_SAM_FineTune"
...
class JSBSimF16SAMEvasionEnv:
    ...
    - agent kinematics (11)
    - relative geo to goal waypoint (10)
    - SAM relative info (4)
...
# PPO Update
# ============================================================
def ppo_update(rollout_buffer, actor, critic, actor_optimizer, critic_optimizer,
               last_value, gamma, lam, clip_eps, ppo_epochs, mini_batch_size,
               entropy_coef, value_coef, max_grad_norm, freeze_actor=False):
    ...
    total_actor_loss = 0.0; total_critic_loss = 0.0; total_entropy = 0.0; update_count = 0
    ...
    for _ in range(ppo_epochs):
        ...
        for start in range(0, n, mini_batch_size):
            ...
            mb_states = states_t[mb_idx]; mb_actions = actions_t[mb_idx]
            mb_old_log_probs = old_log_probs_t[mb_idx]; mb_returns = returns_t[mb_idx]
            ...
            total_actor_loss += actor_loss.item(); total_critic_loss += critic_loss.item()
            total_entropy += entropy.mean().item(); update_count += 1

    cnt = max(update_count, 1)
    return total_actor_loss/cnt, total_critic_loss/cnt, total_entropy/cnt


def smart_load_weights(model, ckpt_path, device, key=None):
    """Load weights with shape-checking; skip mismatched layers (e.g. obs-dim mismatch)."""
    ...
    for k, v in state_dict.items():
        if k in model_dict:
            if v.shape == model_dict[k].shape:
                ...
                skipped_layers.append(f"{k} (ckpt {v.shape} vs model {model_dict[k].shape})")

    model_dict.update(pretrained_dict)
    model.load_state_dict(model_dict)

    print(f"[SmartLoad] Loaded {len(pretrained_dict)}/{len(model_dict)} layers.")
    if skipped_layers:
        print(f"[SmartLoad] Skipped (shape mismatch):")
        for s in skipped_layers:
            print(f"  - {s}")
    else:
        print("[SmartLoad] All layers matched exactly.")
...
def train(args) -> None:
    ...
    mission = SAMEvasionConfig(
        ...
        radar_floor_m=args.radar_floor_m, threat_alpha=args.threat_alpha,
    ...
    dummy = env.reset()
    state_shape = dummy.shape[1:]   # (5, 25)
    state_size = state_shape[1]     # 25
    action_size = 4
    ...
    # Load self-play pretrained actor weights
    if getattr(args, "init_from", None):
        ...
        print(f"[Init] Actor-Only Transfer from Self-Play: {ckpt_path}")
    ...
    rollout_buffer = RolloutBuffer(args.n_steps, state_shape, action_size, device)
    rollout_buffer.reset()

    start_ep = int(getattr(args, "start_episode", 0))
    for episode in range(start_ep, start_ep + int(args.episodes)):

        def get_lr(base_lr: float, ep: int) -> float:
            if not getattr(args, "lr_decay", False):
                return base_lr
            progress = min(ep / max(int(args.episodes)-1, 1), 1.0)
            return max(base_lr*(1.0-progress) + getattr(args, "min_lr", 1e-5)*progress,
                       getattr(args, "min_lr", 1e-5))

        current_entropy_coef = (
            args.entropy_coef
            - (args.entropy_coef - args.entropy_coef_end)
            * (episode / max(int(args.episodes)-1, 1))
        )
        if getattr(args, "lr_decay", False):
            for pg in actor_opt.param_groups: pg["lr"] = get_lr(args.actor_lr, episode)
            for pg in critic_opt.param_groups: pg["lr"] = get_lr(args.critic_lr, episode)
        ...
        last_actor_loss = None; last_critic_loss = None
        total_entropy = 0.0; entropy_count = 0
        ...
        while not done and steps < args.max_steps:
            with torch.no_grad():
                ...
                mu, std = actor(st)
                act_np, log_prob = get_action_ppo(mu, std)
                value = critic(st).squeeze(-1).item()
                act_env = np.asarray(act_np, dtype=np.float32).flatten()
                act_env[:3] = np.clip(act_env[:3], -1, 1)
                act_env[3] = np.clip(act_env[3], 0, 1)
            ...
            traj_wpidx.append(int(info["wp_idx"])); traj_uperr.append(ue); traj_tf.append(tf)

            rollout_buffer.push(state.squeeze(0), act_env, log_prob, float(reward), bool(done), value)
            state = next_state; score += reward; steps += 1

        if len(rollout_buffer) >= args.n_steps:
            with torch.no_grad():
                last_st = torch.tensor(state, dtype=torch.float32, device=device)
                last_val = critic(last_st).squeeze(-1).item() if not done else 0.0

            is_frozen = episode < getattr(args, "freeze_actor_episodes", 0)
            actor_loss, critic_loss, entropy = ppo_update(
                rollout_buffer, actor, critic, actor_opt, critic_opt,
                last_val, args.gamma, args.lam, args.clip_eps, args.ppo_epochs,
                args.mini_batch_size, current_entropy_coef, args.value_coef,
                args.max_grad_norm, freeze_actor=is_frozen)
            last_actor_loss = actor_loss; last_critic_loss = critic_loss
            total_entropy += entropy; entropy_count += 1
            rollout_buffer.reset()
        ...
        try:
            ...
            m_pad = max(r_horiz*1.2, r_s*1.6, 1000)
            all_e = np.concatenate([ae, we, [se]]); all_n = np.concatenate([an, wn, [sn]])
            all_z = np.concatenate([az, wz, [sz]])
            ax3d.set_xlim(all_e.min()-m_pad, all_e.max()+m_pad)
            ax3d.set_ylim(all_n.min()-m_pad, all_n.max()+m_pad)
            ...
            fig.suptitle(f"Episode {episode} - {res} | Score: {score:.1f} | Steps: {steps}", y=0.98, fontsize=14)

            box_txt = (f"PPO SAM Evasion (FineTune)\nKill rH: {r_horiz/1000:.1f}km rV: {r_vert/1000:.1f}km\n"
        ...
        writer.add_scalar("episode/entropy", avg_entropy, episode)
        ...
        print(
            "{} episode | score: {:.2f} | entropy: {:.2f} | min_TF: {:.2f} | radar_steps: {} | range2d: {:.0f}m{}".format(
                episode, score, avg_entropy, min_tf, in_radar_steps, traj_r2d[-1], tag
            )
        )
        ...
        try:
            with metrics_csv.open("a", newline="", encoding="utf-8") as f:
                ...
                w.writerow({
                    "episode": episode, "steps": steps, "score": float(score),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--entropy_coef_end", type=float, default=0.01)
    parser.add_argument("--value_coef", type=float, default=0.5)
    parser.add_argument("--max_grad_norm", type=float, default=0.5)
    parser.add_argument("--lr_decay", action="store_true", default=False)
    parser.add_argument("--min_lr", type=float, default=1e-5)
    parser.add_argument("--freeze_actor_episodes", type=int, default=50,
                        help="Episodes to freeze Actor (only train Critic) before joint fine-tuning.")
    ...
    parser.add_argument(
        "--init_from",
        type=str,
        default="models/20260226_PPO_Self_Play/selfplay_epi_00960.pth",
        help="Path to PPO self-play checkpoint to transfer actor weights from.",
    )
