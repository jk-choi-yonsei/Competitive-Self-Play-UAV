...
SCRIPT_TAG = "20260306_SAC_SAM_FineTune_From_Return"
...
# ============================================================
# SAM Evasion Environment (obs: 25 dim × 5 frames = (1,5,25))
# ============================================================
class JSBSimF16SAMEvasionEnv:
    ...
    NOTE: Return mission obs is 21-dim (no SAM obs).
          smart_load_weights() handles the shape mismatch automatically
          (conv1 will be skipped, upper layers fully transferred).
    """
...
def sac_update(replay_buffer, batch_size, actor, critic, target_critic,
    ...
    c_loss = torch.nn.functional.mse_loss(q1.squeeze(1), target) +              torch.nn.functional.mse_loss(q2.squeeze(1), target)
...
# ============================================================
# Best Checkpoint Finder (Based on moving average success rate)
# ============================================================
def find_best_checkpoint(init_from: str) -> Path:
    """
    If --init_from path is:
      (a) .pth file -> Return as is
      (b) Directory (models/<run>) -> In episode_metrics.csv,
          return .pth of the episode with the highest moving average success rate (window=50)

    CSV search order:
      1. <init_from>/../runs/<run_name>/episode_metrics.csv (Absolute path)
      2. PROJECT_ROOT/runs/<run_name>/episode_metrics.csv (Project relative)
      If CSV is missing, return the latest .pth file in the directory (fallback).
    """
    p = Path(str(init_from))
    if not p.exists():
        p = PROJECT_ROOT / init_from
    if not p.exists():
        raise FileNotFoundError(f"--init_from not found: {init_from}")

    # (a) File specified directly
    if p.is_file():
        return p

    # (b) Directory - Search for best episode in CSV
    run_name = p.name  # e.g. "20260113_SAC_Return"
    csv_candidates = [
        p.parent.parent / "runs" / run_name / "episode_metrics.csv",  # models/../runs/
        PROJECT_ROOT / "runs" / run_name / "episode_metrics.csv",
    ]
    csv_path = next((c for c in csv_candidates if c.exists()), None)
    if csv_path is None:
        # Collect episode_metrics.csv from subfolders in runs/ that contain run_name in their name
        found_csvs = list((PROJECT_ROOT / "runs").rglob(f"*{run_name}*/episode_metrics.csv"))
        if found_csvs:
    ...
    if csv_path is not None:
        try:
            ...
            with csv_path.open("r", encoding="utf-8") as f:
                ...
                for row in reader:
                    rows.append({"episode": int(row["episode"]), "success": float(row["success"])})
                if rows:
                    window = min(50, len(rows))
                    # Calculate moving average success rate
                    best_ep, best_ma = rows[0]["episode"], 0.0
                for i in range(len(rows)):
                    start = max(0, i - window + 1)
                    ma = sum(r["success"] for r in rows[start:i+1]) / (i - start + 1)
                    if ma >= best_ma:
                        best_ma = ma
                        best_ep = rows[i]["episode"]
                ckpt = p / f"epi_{best_ep:05d}.pth"
                if ckpt.exists():
                    print(f"[BestCkpt] CSV: {csv_path.name}")
                    print(f"[BestCkpt] Best episode: {best_ep} (MA-{window} success rate: {best_ma:.3f})")
                    print(f"[BestCkpt] -> {ckpt}")
                    return ckpt
                else:
                    print(f"[BestCkpt] Warning: best ckpt epi_{best_ep:05d}.pth not found in {p}, falling back to latest.")
        except Exception as e:
            print(f"[BestCkpt] Warning: could not parse CSV ({e}), falling back to latest .pth.")
    else:
        print(f"[BestCkpt] No episode_metrics.csv found. Falling back to latest .pth in {p}.")

    # Fallback: 디렉터리에서 가장 최신 .pth
    pths = sorted(p.glob("epi_*.pth"))
    if not pths:
        raise FileNotFoundError(f"No .pth files found in {p}")
    print(f"[BestCkpt] Fallback: using latest checkpoint -> {pths[-1].name}")
    return pths[-1]


# ============================================================
# Smart Weight Loading (shape mismatch 자동 처리)
# Return obs: 21dim vs SAM obs: 25dim -> conv1 shape mismatch -> skip, 나머지 전이
# ============================================================
def smart_load_weights(model, ckpt_path, device, key=None):
    """Load weights from checkpoint. Skips mismatched layers (e.g. conv1 due to obs dim change)."""
    ...
    print(f"[SmartLoad] Loaded {len(pretrained)}/{len(model_dict)} layers.")
    if skipped:
        print("[SmartLoad] SKIPPED layers (shape mismatch — expected for Return->SAM transfer):")
        for s in skipped:
            print(f"  - {s}")
    else:
...
def train(args):
    ...
    mission = SAMEvasionConfig(
        ...
        fail_penalty=args.fail_penalty, crash_penalty=args.crash_penalty,
    )
    ...
    # === Return -> SAM Actor-Only Transfer (Automatic selection of highest success rate checkpoint) ===
    # find_best_checkpoint() finds the episode with the highest MA-50 success rate from episode_metrics.csv.
    # Return obs: (1,5,21), SAM obs: (1,5,25) -> conv1 shape mismatch -> auto-skipped
    if getattr(args, "init_from", None):
        ckpt_path = find_best_checkpoint(args.init_from)
        print(f"[Transfer] Return -> SAM: Actor-Only Transfer from: {ckpt_path}")
        print(f"[Transfer] NOTE: conv1 layer may be skipped due to obs dim change (21->25).")
        smart_load_weights(actor, ckpt_path, device, key="actor")
        print("[Transfer] Critic: random init (no transfer)")
        with torch.no_grad():
            log_alpha.fill_(0.0)
        print(f"[Transfer] Alpha reset to {log_alpha.exp().item():.2f} (max exploration)")
    else:
        print("[Init] No checkpoint specified. Training from scratch.")
    ...
    for episode in range(start_ep, start_ep + int(args.episodes)):
        ...
        while not done and steps < args.max_steps:
            ...
            alt_now = float(env.fdm.get_property_value("position/h-sl-meters"))
            if tf < 4.0 and alt_now > mission.radar_floor_m: in_radar_steps += 1
        ...
        try:
            ...
            ax3d.set_title("3D Trajectory + SAM Threat Dome")
            ax3d.set_xlabel("East (m)"); ax3d.set_ylabel("North (m)"); ax3d.set_zlabel("Up (m)")
            try: ax3d.set_proj_type("ortho")
            except: pass
            ax3d.view_init(elev=22, azim=-55)
            m_lim = max(r_horiz*1.2, r_s*1.6, 1000)
            all_e = np.concatenate([ae, we, [se]]); all_n = np.concatenate([an, wn, [sn]]); all_z = np.concatenate([az, wz, [sz]])
            ax3d.set_xlim(all_e.min()-m_lim, all_e.max()+m_lim)
            ax3d.set_ylim(all_n.min()-m_lim, all_n.max()+m_lim)
            ...
            ax_xz.axhline(y=mission.radar_floor_m - ref_alt, color="orange", lw=1.5, ls="--", alpha=0.7, label=f"Radar Floor ({mission.radar_floor_m:.0f}m)")
            ax_xz.set_title("Side View (XZ)"); ax_xz.grid(True, ls="--", alpha=0.4)

            spd_arr = np.array(traj_spd)
            ax_spd.plot(spd_arr, color="C4", linewidth=2)
        ...
        print(f"{episode} ep | score: {score:.2f} | alpha: {alpha_v:.3f} | entropy: {avg_ent:.2f} "
              f"| min_TF: {min_tf:.2f} | radar_steps: {in_radar_steps} | r2d: {traj_r2d[-1]:.0f}{tag}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=("SAC SAM Evasion Fine-Tune FROM Return-trained Agent.\n"
                     "Hypothesis: Return-trained agents already generalize to SAM evasion.\n"
                     "This script fine-tunes Return weights on SAM env to show Return->SAM transfer works.")
    )
    ...
    parser.add_argument("--radar_floor_m", type=float, default=1500.0)
    ...
    parser.add_argument(
        "--init_from",
        type=str,
        default="models/20260113_SAC_Return_20260119_1057",
        help=("Return trained model directory or .pth file path.\n"
              "When directory is specified: In runs/<name>/episode_metrics.csv,\n"
              "the episode with the highest moving average (window=50) success rate is automatically selected.\n"
              "When .pth file is specified: Use that checkpoint directly."),
    )
